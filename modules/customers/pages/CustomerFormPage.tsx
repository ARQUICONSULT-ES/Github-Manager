...
export function CustomerFormPage({ customerId }: CustomerFormPageProps) {
  ...
  useEffect(() => {
    const checkDuplicateName = async () => {
      ...
      // No validar si el nombre está vacío
      if (!trimmedName) {
        setDuplicateError("");
        return;
      }
      
      // No validar si es el mismo nombre que tenía originalmente (modo edición)
      if (isEditMode && customer && trimmedName.toLowerCase() === customer.customerName.toLowerCase()) {
        setDuplicateError("");
        return;
      }
      ...
      try {
        // Usar el endpoint específico de verificación
        ...
        // No mostrar error al usuario en caso de fallo de red
      } finally {
    ...
    const timeoutId = setTimeout(checkDuplicateName, 500);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar que no haya error de duplicado
    if (duplicateError) {
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          } bg-white dark:bg-gray-900 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:border-transparent transition-all text-xs sm:text-sm placeholder:text-gray-400 dark:placeholder:text-gray-500 ${checkingDuplicate ? 'pr-8' : ''}`}
